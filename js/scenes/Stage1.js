...
export default class Stage1 extends Phaser.Scene {
  ...
  create() {
    ...
    this.enemies = [];
    const positions = [
      { type: 'drunkA', x: 100, y: height - 100 },
      { type: 'drunkB', x: 300, y: height - 100 },
      { type: 'nerdA',  x: 500, y: height - 100 },
      { type: 'nerdB',  x: 700, y: height - 100 },
      { type: 'oldMan', x: 900, y: height - 100 }
    ];
    positions.forEach(p => {
    ...
    // Setup collisions between player and enemies (to restrict overlap)
    this.enemies.forEach(enemy => {
      this.physics.add.collider(this.player, enemy.sprite);
    });

    // HUD texts
    this.healthText = this.add.text(10, 10, '', { fontFamily: 'Arial', fontSize: '20px', color: '#ffffff' });
    this.scoreText = this.add.text(10, 35, '', { fontFamily: 'Arial', fontSize: '20px', color: '#ffffff' });
    this.comboText = this.add.text(10, 60, '', { fontFamily: 'Arial', fontSize: '20px', color: '#ffff00' });
    this.charText = this.add.text(10, 85, 'Character: ' + this.selectedCharacter, { fontFamily: 'Arial', fontSize: '20px', color: '#ffffff' });

    // Health bar graphics
    this.healthBarBg = this.add.rectangle(width / 2 - 150, 10, 300, 15, 0x444444).setOrigin(0, 0);
    this.healthBar = this.add.rectangle(width / 2 - 150, 10, 300, 15, 0xff3333).setOrigin(0, 0);
